...
{
	public static class SearchByCoordinates
	{
		public static void FindRoute(Graph graph, string start, string end)
		{
			Console.WriteLine("Shortest route by coordinates from:" + start + " to:" + end);
			...
			while (queueNodes.Any())
			{
				...
				foreach (var node in tempNode.NodesList)
				{
					if (!node.Expanded && !queueNodes.Contains(node))
					{
						Util.SortByDistance(node, graph.GetNode(end), queueNodes);
					}
				}
			}
		}
